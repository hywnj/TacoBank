...
## **✨ 주요 기능**  
- **더치페이 (정산)**: 영수증 OCR을 통해 품목별 정산 및 N/N 방식의 더치페이 지원  
- **영수증 OCR 인식**: OCR API를 활용한 영수증 데이터 분석 및 처리  
- **송금 기능**: 수취인 확인 및 송금 요청, Idempotency Key로 중복 송금 방지  
- **문자 인증**: 문자 인증 요청 및 검증  
- **멤버 관리**: 회원 정보 조회 및 수정, 비밀번호 변경, 친구 검색  
- **계좌 관리**: 계좌 조회, 메인 계좌 설정, 즐겨찾기 관리  
- **친구 및 그룹 관리**: 친구 및 그룹 신청, 수락, 삭제, 관리  
- **알림 시스템**: 알림 조회, 읽음 처리 및 독촉 알림 전송  
